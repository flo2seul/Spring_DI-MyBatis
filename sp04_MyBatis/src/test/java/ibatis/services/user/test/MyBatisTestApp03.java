...
public class MyBatisTestApp03 {
	...
	public static void main(String[] args) throws Exception{
		...
		System.out.println(":: 0. getUser(SELECT)  ? ");
		List<User> list = session.selectList("User01.getUser");
...
//		Object obj = session.insert("User03.addUser", user);
...
//		System.out.println("\n");
		...
		String userName = (String)session.selectOne("User01.findUserId", user);
		...
		System.out.println(":: 6. getUserList(SELECT)  ? ");
		 list = session.selectList("User01.getUserList");
		for (int i =0 ;  i < list.size() ; i++) {
			System.out.println( "<"+ ( i +1 )+"> 번째 회원.."+ list.get(i).toString() );
		}
	}//end of main
